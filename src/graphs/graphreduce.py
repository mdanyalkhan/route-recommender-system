...
from RoadGraph.graphreduce import RoutesGraph, RoadAssignment
import pickle
...
def generate_routes_graph(road_graph: rg.stdroadgraph, isotrack_df: gpd.GeoDataFrame, out_path: str):
    ...
    out_path = rg.util.create_file_path(f"{out_path}/road_graph")
...
def find_and_save_k_paths(road_graph: rg.stdroadgraph, key_sites: gpd.GeoDataFrame, key_site_col_name: str,
    ...
    out_path = rg.util.create_file_path(f"{out_path}/shortest_paths")

    i = 1
    for path in road_graph.k_shortest_paths_from_key_sites(key_sites, key_site_col_name, source, target, k):
        cur_path = rg.util.create_file_path(f"{out_path}/shortest_path_{i}")
...
def extract_and_save_cluster_gdfs(isotrack_gdf, edges_gdf, nodes_gdf, out_path):
    ...
    cluster_path = rg.util.create_file_path(f"{out_path}/cluster_gdfs")
    for cluster_id in shp_map:
        path = rg.util.create_file_path(f"{cluster_path}/{cluster_id}")
...
if __name__ == '__main__':
    ...
    out_path = rg.util.create_file_path(f"{parent_directory_at_level(__file__, 4)}/Operational_Data/lbb/reduced/HW_SW")
