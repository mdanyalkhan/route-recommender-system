...
class TestMergeNetworkDataFrames(TestCase):
    ...
    to_merge_edges_df = pd.DataFrame({
        INDEX: [0, 1, 2, 3, 4, 5, 6],
        STD_ROAD_NO: ["A1", "X1", "A1", "A2", "A2", "A3", "A4"],
    ...
    def test_roads_are_excluded_between_dataframes(self):
        ...
        new_edges_df, new_nodes_df = NodesAndEdgesMerger()._exclude_roads(base_edges_df,
                                                                          to_merge_edges_df,
                                                                          to_merge_nodes_df)

        self.assertEqual(new_edges_df[STD_ROAD_NO].tolist(), ["A1", "A1", "A2", "A2", "A3", "A4"])
    ...
    def test_road_is_connected_to_roundabout(self):
        ...
        new_edges_df, new_nodes_df, _ = NodesAndEdgesMerger()._connect_by_nodes(base_nodes_df,
                                                                                to_merge_nodes_df,
                                                                                to_merge_edges_df,
                                                                                N_ROUNDABOUT)
    ...
    def test_dead_end_nodes_connect(self):
        ...
        new_edges_df, new_nodes_df, _ = NodesAndEdgesMerger(threshold=0)._connect_by_nodes(base_nodes_df,
                                                                                           to_merge_nodes_df,
                                                                                           to_merge_edges_df,
                                                                                           N_DEAD_END)
    ...
    def test_overall_function(self):
        ...
        base_edges_df, base_nodes_df = NodesAndEdgesMerger(threshold=0).merge_two_network_gdfs(base_edges_df,
                                                                                               base_nodes_df,
                                                                                               to_merge_edges_df,
                                                                                               to_merge_nodes_df)

        self.assertEqual(base_edges_df[STD_ROAD_NO].tolist(), new_road_no)
