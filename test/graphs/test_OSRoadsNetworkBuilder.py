...
class TestOSRoadsNetworkBuilder(TestCase):
    ...
    def test_connection_of_road_segments_is_correct(self):
        df = pd.DataFrame({
            INDEX: [0, 1, 2],
            STD_ROAD_NO: ['A1', 'A1', 'M1'],
            STD_FUNCT_NAME: ["Main Carriageway", "Main Carriageway", "Main Carriageway"],
        ...
        df, _ = OSNodesAndEdgesBuilder()._connect_and_assign_nodes_main_carriageways_slip_roads(df, {})
    ...
    def test_connection_where_there_are_more_than_two_road_segment_connections(self):
        df = pd.DataFrame({
            INDEX: [0, 1, 2, 3, 4, 5],
            STD_ROAD_NO: ['A1', 'A1', 'A1', 'M1', 'M1', 'M1'],
            STD_FUNCT_NAME: ["Main Carriageway", "Main Carriageway", "Main Carriageway", "Main Carriageway",
        ...
        df, nodes = OSNodesAndEdgesBuilder()._connect_and_assign_nodes_main_carriageways_slip_roads(df, {})
    ...
    def test_connections_to_os_type_roundabout(self):
        ...
        gdf, nodes = OSNodesAndEdgesBuilder()._nodes_roads_to_roundabouts(gdf, nodes)
