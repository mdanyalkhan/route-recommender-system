...
class NodesAndEdgesBuilder(ABC):

    def build_nodes_and_edges(self, roads_gdf: gpd.GeoDataFrame, is_directional: bool, node_tag ="")\
    ...
    def _assign_nodes_to_dead_end_roads(self, roads_gdf: gpd.GeoDataFrame,
        ...
        dead_ends = roads_gdf.loc[(roads_gdf[FROM_NODE] == STD_NONE) | (roads_gdf[TO_NODE] == STD_NONE)]
        dead_ends = dead_ends.loc[(pd.isna(roads_gdf[PREV_IND])) | (pd.isna(roads_gdf[NEXT_IND]))]
        dead_ends = dead_ends.loc[(roads_gdf[STD_FUNCT_NAME] == STD_MAIN_CARRIAGEWAY) |
                                  (roads_gdf[STD_FUNCT_NAME] == STD_SLIP_ROAD)]
        for index, dead_end in dead_ends.iterrows():
            if pd.isna(dead_end.PREV_IND) and dead_end.FROM_NODE == STD_NONE:
            ...
            if pd.isna(dead_end.NEXT_IND) and dead_end.TO_NODE == STD_NONE:
